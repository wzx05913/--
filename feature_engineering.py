...
def _freq_features(x: np.ndarray, fs: float, speed_hz: float) -> Dict[str, float]:
    n = len(x)
    if n < 4:
        return {"spectral_centroid": 0.0, "spectral_entropy": 0.0, **{f"amp_{k}": 0.0 for k in ("BPFO", "BPFI", "BSF", "FTF")}}
    w = np.hanning(n)
    ...
    for name, fc in compute_fault_freqs(speed_hz).items():
        mask = (freqs >= fc - 5.0) & (freqs <= fc + 5.0)
        out[f"amp_{name}"] = float(np.max(amp[mask])) if np.any(mask) else 0.0
    return out
...
def extract_base_features(record: WindowRecord, cfg: PipelineConfig) -> Dict[str, float]:
    x = np.asarray(record.history, dtype=float)
    feats: Dict[str, float] = {}
    channel_time = {}
    for ch, cname in enumerate(CHANNEL_NAMES):
        tf = _time_features(x[:, ch]); channel_time[cname] = tf
        ff = _freq_features(x[:, ch], cfg.sampling_rate_hz, record.speed_hz)
...
def extract_features(record: WindowRecord, cfg: PipelineConfig, prev_base: Optional[Dict[str, float]] = None) -> Tuple[Dict[str, float], Dict[str, float]]:
    base = extract_base_features(record, cfg)
    full = dict(base)
    for k, v in base.items():
        full[f"delta_{k}"] = 0.0 if prev_base is None else float(v - prev_base.get(k, 0.0))
    full.update({"run_index": record.window_id, "speed_hz": record.speed_hz, "load_kn": record.load_kn, "condition": record.condition})
    return full, base
